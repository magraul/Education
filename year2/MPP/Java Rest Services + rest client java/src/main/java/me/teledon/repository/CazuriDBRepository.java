...
public class CazuriDBRepository implements CazuriRepository {
    ...
    public CazuriDBRepository() {
        ...
        logger.info("Initializing with properties: {}", serverProps);
    ...
    public List<CazCaritabil> findCazuriDupaDescriere(String descriere) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Integer idd = rs.getInt("id_caz");
                c.setId(idd.toString());
                cazuri.add(c);
            }
            con.close();
    ...
    public CazCaritabil save(CazCaritabil elem) {
        ...
        try {
            if (elem.getId() != null) {
                ...
                st.setInt(1, Integer.parseInt(elem.getId()));
    ...
    public CazCaritabil update(CazCaritabil elem) {
        ...
        try {
            ...
            stmt.setString(1, elem.getDescription());
            stmt.setInt(2, Integer.parseInt(elem.getId()));
            stmt.executeUpdate();
            con.close();
        } catch (SQLException e) {
            logger.traceExit(e);
    ...
    @Override
    public CazCaritabil delete(String elem) throws IllegalArgumentException {
        if (elem == null)
        ...
        try {
            PreparedStatement st = con.prepareStatement("select * from entitati.public.cazuri where id_caz=?");
            st.setInt(1, Integer.parseInt(elem));
            ResultSet r = st.executeQuery();
            ...
            stmt.setInt(1, Integer.parseInt(elem));
            stmt.executeUpdate();
            con.close();
        } catch (SQLException e) {
            logger.traceExit(e);
    ...
    @Override
    public CazCaritabil get(String integer) {
        ...
        try {
            ...
            stmt.setInt(1, Integer.parseInt(integer));
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
                CazCaritabil a = new CazCaritabil(rs.getString("descriere"));
                Integer idd = rs.getInt("id_caz");
                a.setId(idd.toString());
                return a;
            }
            con.close();
        } catch (SQLException e) {
